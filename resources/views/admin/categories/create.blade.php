<x-main-layout>
    <x-slot name="title">Create Category — ChronosLuxury</x-slot>

    <!-- Header Section -->
    <section class="bg-neutral-900 border-b border-white/5 py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
            <div>
                <h1 class="font-serif text-2xl text-white font-bold tracking-wide">Add New Category</h1>
                <p class="text-xs text-luxury-gold tracking-widest uppercase font-semibold mt-1">Expanding Boutique Collections</p>
            </div>
            ...
            <div class="bg-zinc-950 border border-white/5 rounded-lg p-6 sm:p-8">
                ...
                <form action="{{ route('categories.store') }}" method="POST" class="space-y-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" id="name" name="name" value="{{ old('name') }}" required placeholder="e.g. Vintage, Dress, Chronograph..."
                               class="w-full bg-zinc-900 border border-white/10 rounded px-3 py-2 text-xs text-zinc-300 placeholder-zinc-600 focus:outline-none focus:border-luxury-gold focus:ring-0">
                        @error('name')
